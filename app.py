...
def get_bot_response():
    ...
    if s and s.lower().strip() == "exit":
        name = session.get("name", "")
        session.clear()
        return f"Thanks {name} for using the AI Medical Chatbot. Take care 👋"
    ...
    if session['step'] == "FS":
        if s.lower().strip() in ["no", "nah", "nope", "why"]:
            return "Please describe a symptom properly (example: fever, headache)."
        sym1 = s
    ...
    if session['step'] == 'PD':
        ...
        if "all" not in session:
            ...
            if not diseases:
                session['step'] = "Q_C"
                return (
                    "Based on the symptoms you confirmed, I’m unable to confidently "
                    "map them to a known condition.<br>"
                    "Could you describe any other symptom you’re experiencing?<br>"
                    "Or type <b>exit</b> to end the consultation."
                )

            dis = diseases[0]
            session["dis"] = dis
            session['step'] = "for_dis"
            # Initialize follow-up symptom list for the disease
            session["symv"] = symVONdisease(df_tr, session["dis"])


       # Follow-up symptom confirmation
        ...
        if len(session["symv"]) > 0:
            ...
            if len(s) > 0:
                ...
                # Remove current symptom
                symts.pop(0)
                session["symv"] = symts

            # Ask next symptom
            if len(session["symv"]) > 0:
                next_sym = session["symv"][0]
                if next_sym not in session["all"] and next_sym not in session["asked"]:
                    return "Are you experiencing " + clean_symp(next_sym) + "?"
